...
const apply=(el,dx=0,dy=0,scale=1)=>{el.style.transform=`scale(${scale}) ${baseOf(el)} translate(${dx}px,${dy}px)`;}
...
export function wireHandDrag(root, dispatch){
  ...
  root.addEventListener('pointerdown',e=>{
    ...
    const onMove=ev=>{const dx=ev.clientX-sx, dy=ev.clientY-sy; if(!dragging && (Math.abs(dx)>5||Math.abs(dy)>5)){clearTimeout(pressTimer); el.classList.remove('is-preview'); dragging={el,id}; toggle(root,el.dataset.type,true,el); el.setPointerCapture(ev.pointerId); el.classList.add('is-dragging'); el.style.pointerEvents='none'; } if(dragging){ apply(el,dx,dy,1.04);} };
    ...
    root.addEventListener('pointermove',onMove,{passive:true}); root.addEventListener('pointerup',onUp,{passive:true}); root.addEventListener('pointercancel',onCancel,{passive:true});
    window.addEventListener('pointerup',onUp,{once:true}); window.addEventListener('pointercancel',onCancel,{once:true}); window.addEventListener('blur',onCancel,{once:true}); }, {passive:true}); }