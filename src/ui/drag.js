// =========================================================
// THE GREY — Drag & Drop (v2.2)
// • Highlights valid targets (Spell slots vs Glyph tray)
// • Instants: self pulse (no drop targets)
// • On drop, writes target slot index to the card dataset so
//   UI/index.js can honor exact placement.
// =========================================================
...
(function () {
  ...
  function pointerUp() {
    ...
    if (overSlot && overSlot.classList.contains('drop-ok')) {
      // Pass the target slot index to the UI via dataset
      const idx = overSlot.dataset.slotIndex;
      node.dataset.dropSlot = (idx !== undefined ? String(idx) : '');
  ...
  function attach() {
    ...
    console.log('[Drag] initialized (v2.2): typed targets + dataset dropSlot');
  }
