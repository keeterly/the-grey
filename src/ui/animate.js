    ...
    switch (a.type) {
      ...
      case 'DISCARD_HAND': {
        ...
        for (const el of cards) {
          ...
          const ghost = document.createElement('div'); ghost.className = 'fly';
          ...
          ghost.style.transform = `translate(${tx}px, ${ty}px) scale(.6) rotate(${(i%2?8:-8)}deg)`; ghost.style.opacity = '0';
          await wait(240); ghost.remove(); i++;
        }
        discard.classList.add('highlight'); await wait(160); discard.classList.remove('highlight');
        break;
      }
      case 'ADVANCE': {
