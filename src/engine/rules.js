...
export function reducer(state, action){
  ...
  switch(action.type){
    ...
    case A.DISCARD_FOR_AETHER: { const {cardId, who='YOU'}=action; const tgt=who==='YOU'?you:ai; const i=tgt.hand.findIndex(c=>c.id===cardId); if(i<0) return s; const card=tgt.hand.splice(i,1)[0]; tgt.aether += (card.aetherGain||0); tgt.discard.push(card); s.animations.push({type:'PLAY', who, cardId:card.id}); return s; }
    ...
    case A.AI_TURN: {
      const gi=ai.hand.findIndex(c=>c.type===CARD_TYPES.GLYPH); if(!ai.glyphSlot && gi>=0){ const g=ai.hand.splice(gi,1)[0]; if(ai.glyphSlot) ai.discard.push(ai.glyphSlot); ai.glyphSlot=g; s.animations.push({type:'PLAY_GLYPH','who':'AI', cardId:g.id}); }
      ...
      for(let k=0;k<8;k++){ const idx=ai.slots.findIndex(c=>c && c.type===CARD_TYPES.SPELL && c.pips<c.pipsMax && ai.aether>=c.aetherCost); if(idx===-1) break; const card=ai.slots[idx]; ai.aether -= card.aetherCost; card.pips++; s.animations.push({type:'ADVANCE','who':'AI', cardId:card.id}); if(card.pips===card.pipsMax){ castIfComplete(ai,card,'AI',s.animations); ai.discard.push(card); ai.slots[idx]=null; } }
