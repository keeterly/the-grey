...
export function reducer(state, action){
  ...
  switch(action.type){
    ...
    case A.PLAY_TO_SLOT: {
      ...
      card.pips=0; you.slots[slotIndex]=card; s.animations.push({type:'PLAY', who, cardId:card.id}); return s;
    }
    case A.PLAY_TO_GLYPH: {
      ...
      if (you.glyphSlot) you.discard.push(you.glyphSlot); you.glyphSlot=card; s.animations.push({type:'PLAY_GLYPH', who, cardId:card.id}); return s;
    }
    case A.ADVANCE_PIP: {
      ...
      if (card.pips===card.pipsMax){ castIfComplete(you,card,'YOU',s.animations); you.discard.push(card); you.slots[slotIndex]=null; }
      return s;
    }
