...
export function reducer(state, action){
  ...
  switch(action.type){
    ...
    case A.DISCARD_FOR_AETHER: {
      ...
      s.animations.push({type:'PLAY', who, cardId:card.id}); // small flash
      return s;
    }
    case A.PLAY_TO_SLOT: {
      ...
      if (card.type===CARD_TYPES.INSTANT) { // instants just resolve
        s.animations.push({type:'PLAY', who, cardId:card.id}); castIfComplete(tgt,card,who,s.animations); tgt.discard.push(card); return s;
      }
    ...
    case A.PLAY_TO_GLYPH: {
      const { cardId, who='YOU' } = action; const tgt=who==='YOU'?you:ai;
      const idx=tgt.hand.findIndex(c=>c.id===cardId); if (idx<0) return s;
      const card = tgt.hand.splice(idx,1)[0]; if (card.type!==CARD_TYPES.GLYPH){ tgt.hand.push(card); return s; }
      if (tgt.glyphSlot) tgt.discard.push(tgt.glyphSlot); tgt.glyphSlot=card;
      s.animations.push({type:'PLAY_GLYPH', who, cardId:card.id}); return s;
    }
    case A.ADVANCE_PIP: {
      const { who='YOU', slotIndex } = action; const tgt=who==='YOU'?you:ai;
      const card = tgt.slots[slotIndex]; if (!card || card.type!==CARD_TYPES.SPELL) return s;
    ...
    case A.END_TURN: { discardHand(you); s.turn='AI'; return s; }
    case A.AI_TURN: {
      const t = ai;
      if (!t.glyphSlot){ const gi=t.hand.findIndex(c=>c.type===CARD_TYPES.GLYPH); if(gi>=0){ const g=t.hand.splice(gi,1)[0]; if(t.glyphSlot) t.discard.push(t.glyphSlot); t.glyphSlot=g; s.animations.push({type:'PLAY_GLYPH', who:'AI', cardId:g.id}); } }
      const empty = t.slots.findIndex(x=>!x); const si = t.hand.findIndex(c=>c.type===CARD_TYPES.SPELL);
      if (empty>=0 && si>=0){ const sp=t.hand.splice(si,1)[0]; sp.pips=0; t.slots[empty]=sp; s.animations.push({type:'PLAY', who:'AI', cardId:sp.id}); }
      const ii=t.hand.findIndex(c=>c.type===CARD_TYPES.INSTANT); if(ii>=0){ const inst=t.hand.splice(ii,1)[0]; t.aether += inst.aetherGain||0; t.discard.push(inst); s.animations.push({type:'PLAY', who:'AI', cardId:inst.id}); }
      for (let k=0;k<5;k++){ const idx=t.slots.findIndex(c=>c && c.type===CARD_TYPES.SPELL && c.pips < c.pipsMax && t.aether >= c.aetherCost); if(idx===-1) break;
      ...
      discardHand(t); s.turn='YOU'; if(!you.hand.length) drawN(you,5); return s;
    }
    default: return s;
  }
}