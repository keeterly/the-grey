import { CARD_TYPES, channelCost } from './state.js';
export const A = { START:'START', DRAW:'DRAW', PLAY_TO_SLOT:'PLAY_TO_SLOT', PLAY_TO_GLYPH:'PLAY_TO_GLYPH', ADVANCE_PIP:'ADVANCE_PIP', DISCARD_FOR_AETHER:'DISCARD_FOR_AETHER', CHANNEL_AF:'CHANNEL_AF', END_TURN:'END_TURN', AI_TURN:'AI_TURN' };
...
export function reducer(state,action){
  ...
  switch(action.type){
    ...
    case A.PLAY_TO_SLOT: { const {cardId,who='YOU',slotIndex=0}=action; if(who!=='YOU') return s; const idx=you.hand.findIndex(c=>c.id===cardId); if(idx<0) return s; const card=you.hand.splice(idx,1)[0]; if(card.type!=='SPELL'){ you.hand.push(card); return s; } const prev=you.slots[slotIndex]; if(prev) you.discard.push(prev); card.pips=0; you.slots[slotIndex]=card; s.animations.push({type:'PLAY',who,cardId:card.id}); return s; }
    case A.PLAY_TO_GLYPH:{ const {cardId,who='YOU'}=action; if(who!=='YOU') return s; const idx=you.hand.findIndex(c=>c.id===cardId); if(idx<0) return s; const card=you.hand.splice(idx,1)[0]; if(card.type!=='GLYPH'){ you.hand.push(card); return s; } if(you.glyphSlot) you.discard.push(you.glyphSlot); you.glyphSlot=card; s.animations.push({type:'PLAY_GLYPH',who:'YOU',cardId:card.id}); return s; }
    case A.ADVANCE_PIP:{ const {slotIndex}=action; const card=you.slots[slotIndex]; if(!card||card.type!=='SPELL') return s; if(you.aether<card.aetherCost) return s; you.aether-=card.aetherCost; card.pips=Math.min(card.pips+1,card.pipsMax); s.animations.push({type:'ADVANCE',who:'YOU',cardId:card.id}); if(card.pips===card.pipsMax){ castIfComplete(you,card,'YOU',s.animations); you.discard.push(card); you.slots[slotIndex]=null; } return s; }
    case A.CHANNEL_AF:{ const {afId}=action; const i=s.aetherflow.findIndex(c=>c.id===afId); if(i<0) return s; const card=s.aetherflow[i]; const cost=channelCost(card); if(you.aether < cost) return s; you.aether -= cost; const playerCard={...card,id:card.id.replace(/^AF\d+-/,'PF')}; you.discard.push(playerCard); s.animations.push({type:'CHANNEL_AF',who:'YOU',cardId:card.id}); if(s.aetherflowPool.length){ s.aetherflow[i]=s.aetherflowPool.shift(); } else { s.aetherflow.splice(i,1);} return s; }
    case A.END_TURN:{ discardHand(you,s.animations,'YOU'); s.turn='AI'; return s; }
    case A.AI_TURN:{ const idxAffordable = s.aetherflow.findIndex(c=> ai.aether >= channelCost(c)); if (idxAffordable>=0){ const c=s.aetherflow[idxAffordable]; const cost=channelCost(c); ai.aether-=cost; const aiCard={...c,id:c.id.replace(/^AF\d+-/,'AF_AI')}; ai.discard.push(aiCard); s.animations.push({type:'CHANNEL_AF',who:'AI',cardId:c.id}); if(s.aetherflowPool.length){ s.aetherflow[idxAffordable]=s.aetherflowPool.shift(); } else { s.aetherflow.splice(idxAffordable,1);} }
      const empty=ai.slots.findIndex(x=>!x); const si=ai.hand.findIndex(c=>c.type==='SPELL'); if(empty>=0&&si>=0){ const sp=ai.hand.splice(si,1)[0]; sp.pips=0; ai.slots[empty]=sp; s.animations.push({type:'PLAY',who:'AI',cardId:sp.id}); }
